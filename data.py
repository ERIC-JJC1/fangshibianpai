...
def data_from_pickles(folder, num_nfeat, num_efeat, num_nmeas, num_emeas, meas_v, meas_pflow):
    ...
    # 获取所需的噪声列  
    v_noise = df_noise_list['v_noise'].values  
    p_noise = df_noise_list['p_noise'].values  
    pm_noise = df_noise_list['PM_NOISE'].values if 'PM_NOISE' in df_noise_list.columns else p_noise  

    # 创建 nodes_noises 数组  
    nodes_noises = np.column_stack([v_noise, v_noise, p_noise, pm_noise]) 
